...
TEST_CASE("a Gauss-Newton minimizer using moving weights and reference-image gradients can be instantiated") {
  typedef float dataT;
  ...
  WARN("elapsed time computing gradient and Hessian: "
    << gradientAndHessianComputeTime << " ms");
      ...
      SECTION(std::string("and registering two images ") +
        ...
        REQUIRE(6 * sizeof(dataT)
              == BinaryFile< std::vector<dataT> >::read(&paramSolution,
                  "Moving_Weighted_Gauss_Newton_Ref_Grad_tests/parameterOutput.dat"));
      ...
      SECTION(std::string("and registering two images ") +
        ...
        REQUIRE(6 * sizeof(dataT)
              == BinaryFile< std::vector<dataT> >::read(&paramSolution,
                  "Moving_Weighted_Gauss_Newton_Ref_Grad_tests/accumulateParameterOutput.dat"));
