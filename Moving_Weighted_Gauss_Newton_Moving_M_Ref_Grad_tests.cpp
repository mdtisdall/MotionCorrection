...
TEST_CASE("a Gauss-Newton minimizer using moving weights, moving M, and reference-image gradients can be instantiated") {
  typedef float dataT;
  ...
  MinimizerT minimizer(
    &interpolator, &dz, &dy, &dx, &weightFunc, &weightGradientFunc);
...
TEST_CASE("a Gauss-Newton minimizer using moving weights, moving M, and reference-image gradients can be instantiated from image data") {
  ...
  WARN("elapsed time computing gradient and Hessian: "
    << gradientAndHessianComputeTime << " ms");
      ...
      SECTION(std::string("and registering two images ") +
        ...
        REQUIRE(6 * sizeof(dataT)
              == BinaryFile< std::vector<dataT> >::read(&paramSolution,
                  "Moving_Weighted_Gauss_Newton_Moving_M_Ref_Grad_tests/parameterOutput.dat"));
      ...
      SECTION(std::string("and registering two images ") +
        ...
        REQUIRE(6 * sizeof(dataT)
              == BinaryFile< std::vector<dataT> >::read(&paramSolution,
                  "Moving_Weighted_Gauss_Newton_Moving_M_Ref_Grad_tests/accumulateParameterOutput.dat"));
