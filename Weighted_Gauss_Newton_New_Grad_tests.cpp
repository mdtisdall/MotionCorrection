...
TEST_CASE("a weighted Gauss-Newton minimizer using new-image gradients can be instantiated from image data") {
  ...
  SECTION(std::string("and registering two images ") +
    ...
    REQUIRE(6 * sizeof(dataT)
          == BinaryFile< std::vector<dataT> >::read(&paramSolution,
              "Weighted_Gauss_Newton_New_Grad_tests/gradientUpdateParameterOutput.dat"));
  ...
  SECTION(std::string("and registering two images ") +
    ...
    REQUIRE(6 * sizeof(dataT)
          == BinaryFile< std::vector<dataT> >::read(&paramSolution,
//              "Weighted_Gauss_Newton_New_Grad_tests/gradientUpdateParameterOutput.dat"));
              "Weighted_Gauss_Newton_New_Grad_tests/gradientUpdateComposeAccumulateParameterOutput.dat"));
