...
TEST_CASE("a Gauss-Newton minimizer using moving weights, fixed M, and reference-image gradients can be instantiated") {
  typedef float dataT;
...
TEST_CASE("a Gauss-Newton minimizer using moving weights, fixed M, and reference-image gradients can be instantiated from image data") {
  ...
  WARN("elapsed time computing gradient and Hessian: "
    << gradientAndHessianComputeTime << " ms");
      ...
      SECTION(std::string("and registering two images ") +
        ...
        REQUIRE(6 * sizeof(dataT)
              == BinaryFile< std::vector<dataT> >::read(&paramSolution,
                  "Moving_Weighted_Gauss_Newton_Fixed_M_Ref_Grad_tests/parameterOutput.dat"));
      ...
      SECTION(std::string("and registering two images ") +
        ...
        REQUIRE(6 * sizeof(dataT)
              == BinaryFile< std::vector<dataT> >::read(&paramSolution,
                  "Moving_Weighted_Gauss_Newton_Fixed_M_Ref_Grad_tests/accumulateParameterOutput.dat"));
